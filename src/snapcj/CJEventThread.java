 ...
 */
class CJEventThread extends Thread {
    ...
    private void runEvent(Runnable aRun)
    {
        ...
        // If exception is thrown, send to dev console
            ...
            // Get all windows
            WindowView[] winViews = null; //CJScreen.get().getWindows().toArray(new WindowView[0]);
            if (winViews.length < 1) {
                System.err.println("CJEventThread: Uncaught exception, no windows");
                return;
            }
