 ...
 */
public class CJWebSite extends WebSite {
    ...
    // The shared site
    private static CJWebSite _shared;
    ...
    protected CJWebSite()
    {
        ROOT_URL = CJViewEnv.getScriptRoot(); // Was "http://localhost"
     ...
     */
    private static byte[] getBytes(URL aURL)
    {
     ...
     * Returns bytes for url.
     */
    private static byte[] getBytesOrThrow(URL aURL) throws java.io.IOException
    {
...
//        // Split
//        String text = req.getResponseText();
        String[] pathStrings = null; //text.split("\\s*\n\\s*");
    ...
    public URL getJavaURL(Class<?> aClass, String aPath)
    {
        ...
        try {
            URL url = new URL(urls); // was "http://localhost"
     ...
     */
    public static CJWebSite get()
    {
        if (_shared != null) return _shared;
        return _shared = new CJWebSite();
    }
}